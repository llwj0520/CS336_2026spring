...
def parse_args():
    ...
    parser.add_argument(
        "--backward",
        action="store_true",
        help="Measure forward and backward.",
    )
    parser.add_argument(
        "--optimizer-step",
        action="store_true",
        help="Measure forward, backward, and AdamW step.",
    )
    parser.add_argument(
        "--mixed-precision",
        action="store_true",
    ...
    parser.add_argument(
        "--compile",
        action="store_true",
        help="Compile the complete Transformer model.",
    )
    parser.add_argument(
        "--memory-profile",
        choices=["forward", "training"],
        default=None,
        help="Export a CUDA memory snapshot.",
    )
    parser.add_argument(
        "--memory-snapshot",
        default="memory_snapshot.pickle",
        help="Output path for the CUDA memory snapshot.",
    )
...
def benchmark(args):
    if not torch.cuda.is_available():
        raise RuntimeError(
            "This benchmark requires an NVIDIA CUDA GPU."
        )
...
def profile_memory(args):
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA memory profiling requires an NVIDIA CUDA GPU."
        )
